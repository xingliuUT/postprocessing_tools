...
# Note 1: output geometry file is good for local runs, but 
# miss two lines for global runs
# Note 2: geometry file doesn't change with kymin or nx0
if 'x_local' in pars:
    if pars['x_local'] != 'F':
        x_local = True
    else:
...
if x_local:
    ...
    geom_type = pars['magn_geometry'][1:-1]
    geom_file = geom_type + suffix
#    geom_pars, geom_coeff = read_geometry_lilo(geom_file)
    ...
    if 1 == 1:
        plt.plot(geom_coeff['gxx'])
        plt.title('gxx (before)')
        plt.show()

        gxx = geom_coeff['gxx']
        smooth = False
        gxx_smooth = gxx[:,0].copy()
        while not smooth:
            selection = int(raw_input\
                ('Smoothed?\n1.True\n2.False\n'))
            if selection == 1:
            ...
            gxx_smooth = smoothWdiff\
                (gxx[:,0],smoothRate,numIteration)
            gxx_smooth = np.real(gxx_smooth)

            plt.plot(gxx[:,0],label='before')
            plt.plot(gxx_smooth,label='after')
            plt.title('gxx')
            plt.legend()
            plt.show()

        for i in range(pars['nx0']):
            geom_coeff['gxx'][:,i] = gxx_smooth

    if 1 == 1:
        plt.plot(geom_coeff['gxz'])
        plt.title('gxz (before)')
        plt.show()

        gxz = geom_coeff['gxz']
        smooth = False
        gxz_smooth = gxz[:,0].copy()
        while not smooth:
            selection = int(raw_input\
                ('Smoothed?\n1.True\n2.False\n'))
            if selection == 1:
            ...
            gxz_smooth = smoothWdiff\
                (gxz[:,0],smoothRate,numIteration)
            gxz_smooth = np.real(gxz_smooth)

            plt.plot(gxz[:,0],label='before')
            plt.plot(gxz_smooth,label='after')
            plt.title('gxz')
            plt.legend()
            plt.show()

        for i in range(pars['nx0']):
            geom_coeff['gxz'][:,i] = gxz_smooth

    file_name = 'smooth_'+geom_type+suffix
#    write_tracer_efit_file_lilo(geom_pars,geom_coeff,file_name)
    ...
    if 1 == 1:
        ...
        dBdx_smooth = np.zeros((int(pars['nz0']), \
            int(pars['nx0'])), dtype='float')
        for i in range(int(pars['nx0'])):
    ...
    if 1 == 1:
        ...
        dBdz_smooth = np.zeros((int(pars['nz0']), \
            int(pars['nx0'])),dtype='float')
        for i in range(int(pars['nx0'])):
